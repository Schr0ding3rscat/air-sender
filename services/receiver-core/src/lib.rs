...
pub fn app(state: AppState) -> Router {
    Router::new()
        ...
        .route("/v1/debug/snapshot", get(get_debug_snapshot))
...
async fn get_debug_snapshot(State(state): State<AppState>) -> Json<DebugSnapshot> {
    ...
    let sessions = state.sessions.read().await.values().cloned().collect();
    let trusted_devices = state.trusted_devices.read().await.iter().cloned().collect();
    let recordings = state.recordings.read().await.values().cloned().collect();
    let policy = state.policy.read().await.clone();
    let audit = state.audit_log.read().await;
    let mut audit_tail: Vec<AuditEvent> = audit.iter().rev().take(50).cloned().collect();
    audit_tail.reverse();
...
mod tests {
    ...
    #[tokio::test]
    async fn debug_snapshot_endpoint_works() {
        let app = app(AppState::bootstrap("token".into()));
        let response = app
            .oneshot(
                axum::http::Request::builder()
                    .uri("/v1/debug/snapshot")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
    ...
    #[tokio::test]
    async fn health_detail_endpoint_works() {
        let app = app(AppState::bootstrap("token".into()));
        let response = app
            .oneshot(
                axum::http::Request::builder()
